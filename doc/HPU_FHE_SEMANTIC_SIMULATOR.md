...
- 为NTT/INTT生成pre/postfactor和每stage固定`N/2`个物理twiddle。
...
比较器检查instruction index、DMAindex、机器码、助记符、活动模数、变更对象、结果checksum、DDRspan和状态，并报告首条差异记录。
...
- `summary.json`给出DDR双checksum、变化word数量、结果word数量和首个差异。

每条trace包含机器码、解码字段、活动MOD_ID/q、live对象、变更对象、变更span、before/afterchecksum、DDRspan、stage信息和状态。

## 5.数据规模和安全规则

- DDR工作镜像使用`mmap`。
- DDR复制、checksum和diff按4MiB分块。
- 多项式对象使用`array('I')`。
- 不创建完整DDR的Python整数列表。
- 输出目录必须为全新路径。
...
- 测试使用`/tmp/hpu_*`前缀的独立目录并保留产物，便于失败复查；测试代码不执行递归清理。需要回收时按明确文件路径逐个处理。
