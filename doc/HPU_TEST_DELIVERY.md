...
- `test_data/expected_decode.csv`：逐条期望 word、command26、`custom2/custom1` 路由和归一化汇编。
- `test_data/expected_cmd26.csv`：逐条验证 `cmd26[25]=custom_kind`，以及 custom2/custom1 的 `inst[31:7]` payload 原样直通。
