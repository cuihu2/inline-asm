...
默认 61-bit 辅助素数路径不直接假设一个素数完整贡献 61 位，而使用每个素数至少
大于 `2^60` 的事实，按每个辅助素数至少贡献 60 位计算：
...
当前 HPU profile 启用 `SEAL_EXPERIMENTAL_BFV_HPU_32BIT_AUX`，把 SEAL 内部
`B`、`m_sk` 和 `gamma` 都生成为可装入 uint32 的 32-bit 素数。算法约束的是
`B=prod(b_j)` 的总容量，而不是单个 `b_j` 的位宽，因此实现通过增加 RNS limb
数量提供相同范围，不会在线构造多精度大模数。
...
当 `t_bits = 17`、每个 `q_i` 约 31 位、三个 HPU 选项同时开启时，保守参考如下：
...
该表使用最保守的 31 位贡献，正是当前 `RNSTool` 的 sizing gate。参数初始化还会
用生成出的具体素数构造真实乘积 `B`；数学上只需满足：

\[
\log_2 B>R
\]

即可停止增加素数。未来可以用真实乘积收紧数量，但当前实现有意保留上述保守界，
不因具体素数恰好接近 `2^32` 而少分配一个 limb。
